...
// Use isolated temp HOME to avoid touching real ~/.ai-pixel-office/
let tmpBase: string;
...
const { AiPixelOfficeServer } = await import('../src/server.js');
...
describe('AiPixelOfficeServer', () => {
  let server: InstanceType<typeof AiPixelOfficeServer>;

  beforeEach(() => {
    tmpBase = fs.mkdtempSync(path.join(os.tmpdir(), 'pxl-server-test-'));
    serverJsonDir = path.join(tmpBase, '.ai-pixel-office');
    serverJsonPath = path.join(serverJsonDir, 'server.json');
    fs.mkdirSync(serverJsonDir, { recursive: true });
    server = new AiPixelOfficeServer();
  });
  ...
  it('second instance reuses existing server', async () => {
    ...
    const server2 = new AiPixelOfficeServer();
